...
def pygui(stuNo, pyfilename, qNo):
    ...
    #读取文件内容并展示出来
    file=open("python"+str(qNo+1)+"/"+pyfilename,'r',encoding='utf-8')
    ...
    b1 = Button(root,text='复制到剪切板',command=lambda: testcode(pyfilename), width=10,height=2)
    b1.grid(row=20, column=0, padx=5,pady=5)
    #调用给分函数，用于给学生评分
    b2 = Button(root,text='0分',command=lambda: marks_1(stuNo,qNo), width=8,height=2)
    b2.grid(row=20, column=1, padx=5,pady=5)
    b3 = Button(root,text='0.5分',command=lambda: marks_2(stuNo,qNo),width=8,height=2)
    b3.grid(row=20, column=2, padx=5,pady=5)
    b4 = Button(root,text='1分',command=lambda: marks_3(stuNo,qNo),width=8,height=2)
    b4.grid(row=20, column=3, padx=5,pady=5)
    #退出图形界面
    b5 = Button(root,text='下一个',command=lambda: marks_4(stuNo,qNo), width=8,height=2)
